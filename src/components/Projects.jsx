...
const projectsData = [
  ...
  { id: 7, title: 'Fitness Tracker App', category: 'Mobile', image: 'https://plus.unsplash.com/premium_photo-1681433383783-661b519b154a?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8Zml0bmVzcyUyMHRyYWNrZXJ8ZW58MHx8MHx8fDA%3D' },
        ...
        <AnimatePresence>
          ...
          >
            {filteredProjects.map((project, index) => (
              <motion.div
                key={project.id}
                className={`bg-[#1A1A1A] rounded-2xl overflow-hidden shadow-lg ring-1 ring-primary/5 ${
                  activeCategory === 'All' ? getProjectSize(index) : ''
                }`}
              ...
              >
                <div className="relative h-full group">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/60 to-transparent p-5 flex flex-col justify-end transform transition-all duration-300 translate-y-2 group-hover:translate-y-0">
                    <h3 className="text-xl font-semibold text-white mb-2 group-hover:text-primary transition-colors duration-300">
                      {project.title}
                    </h3>
                    <p className="text-white font-semibold text-sm mb-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      {project.category}
                    </p>
                    <p className="text-zinc-300 text-sm mb-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 line-clamp-2">
                      {project.description}
                    </p>
                    <motion.div
                      initial={{ opacity: 0 }}
                    ...
                    >
                      <a
                        href="#"
                        className="inline-flex items-center text-sm text-white hover:text-primary transition-colors duration-300"
                      >
                        View Project <ExternalLink className="ml-1" size={14} />
                      </a>
                    </motion.div>
                  </div>
                </div>
